...
|---|---|---|
| **Solo** | $49 | 1 site, 1 custom domain, DDF listing sync, leads + Follow Up Boss, 1 member, both templates |
| **Team** | $129 | Everything in Solo, plus up to 10 members, agent profile pages + Team block |
...
sharply reduces dunning surface for the pilot.
...
Plans map to entitlement values via the catalog. Server-side mutations and the worker both call the
resolver — enforcement is **server-side**, never merely hidden in UI (M6 acceptance criterion). A
new `billing` RBAC resource gates *managing* the subscription (owner/admin), separate from the
entitlements a subscription *grants*.
...
- **N = 14 days** grace by default (provisional; matches Stripe smart-retry windows).
...
5. **M6-A5** enforcement flip: permissive → enforcing on publish / add-domain / connect-integration /
   invite-member, with UI upsell states.
6. **M6-A6** ops: tenant↔customer reconciliation, event history, comp/extend in super-admin console.

## Open decisions for the owner

- Confirm or replace provisional pricing ($49 Solo / $129 Team CAD).
- Confirm grace window **N = 14 days** and the leads-off point (`past_due` vs `grace_end`).
- Confirm Team member cap (10) and whether custom domains beyond 1 are ever in scope for MVP.
